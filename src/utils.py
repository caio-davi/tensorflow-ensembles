...
def removenan(t):
    """Remove `nan`. **CAUTION**: The returned tensor might not have the same shape from teh input.
    ...
    Returns:
        tf.Tensor: Tensor with `0s` instead of `nan`.
    """
    temp = tf.where(tf.math.is_nan(t), tf.zeros_like(t), t)
    return tf.boolean_mask(temp, tf.reduce_mean(temp, 1))
...
def build_normalized(pop_size, layer_sizes, method):
...
def build_uniform(pop_size, layer_sizes, _):
    x_min = -1
    x_max = 1
...
def build_logLogistic(pop_size, layer_sizes, _):
    dim = dimensions(layer_sizes)
...
def build_NN(pop_size, layer_sizes, method):
    methods = {
        ...
        "log_logistic": "build_logLogistic",
        "xavier": "build_normalized",
        "he": "build_normalized",
        "lecun": "build_normalized",
    }
    return getattr(utils, methods.get(method, "build_normalized"))(
...
def get_splits(size, cuts):
    """Auxiliary function to find milestones over a range.

    Args:
        size (int): Size of the range.
        cuts (int): Number of milestones desired.
